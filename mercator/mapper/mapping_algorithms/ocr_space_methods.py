...
def parse_loc(loc_of_first_word_in_line):
    ...
    x_center_coord = pixels_from_left_side_of_image + \
        math.floor(pixels_width_of_image/2)
    y_center_coord = pixels_from_top_side_of_image + \
        math.floor(pixels_height_of_image/2)
...
def ocr_space_text_map(json_file_to_parse):
    ...
    with open(json_file_to_parse, 'r+') as f:
        ...
        for line_text_and_loc in line_texts_and_locs:
            ...
            x_center_coord, y_center_coord = parse_loc(
                loc_of_first_word_in_line)
...
def ocr_request_error(json_of_ocr_result):
    ...
    with open(json_of_ocr_result, 'r+') as f:
        ...
        if data['IsErroredOnProcessing']:
            print(
                f"One of the requested images, with filename {json_of_ocr_result[0:len(json_of_ocr_result)-5]}, produced an error on processing by ocr.space. ")
